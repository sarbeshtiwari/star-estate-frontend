...
function AppContent() {
  ...
  useEffect(() => {
    // Check if the user is on the home page and if they have accepted the disclaimer
    const hasAccepted = sessionStorage.getItem('landingAccepted');
    if (location.pathname === '/') {
      setShowLanding(!hasAccepted); // Show LandingWrapper if on home page and not accepted
    } else {
      setShowLanding(false); // Hide LandingWrapper if not on home page
    }
  }, [location.pathname]); // Update effect on path change
